...
class QLearningAgent(Node):
    def __init__(self):
        ...
        self.num_buckets = [10, 10, 10, 10]  # Number of discrete bins per state dimension
        self.state_bounds = np.array([
            [0, 5],  # Bounds for state 1
            [-3.15, 3.15],  # Bounds for state 2
            [0, 5],  # Bounds for state 3
            [-3.15, 3.15]   # Bounds for state 4
        ])

        # Q-table
        self.q_table = np.zeros(self.num_buckets + [self.action_size])
    ...
    def get_action(self, state):
        ...
        if np.random.rand() <= self.epsilon:
            ...
            self.q_val = self.q_table[tuple(state_index)]
            return np.argmax(self.q_table[tuple(state_index)])

    def update_q_table(self, state, action, reward, next_state, done):
        state_index = self.discretize_state(state)
        next_state_index = self.discretize_state(next_state)
        q_value = self.q_table[tuple(state_index)][int(action)]
        if done:
            target = reward
        else:
            target = reward + self.discount_factor * np.max(self.q_table[tuple(next_state_index)])
        self.q_table[tuple(state_index)][action] += self.learning_rate * (target - q_value)

    def discretize_state(self, state):
        ratios = [(state[i] - self.state_bounds[i][0]) / (self.state_bounds[i][1] - self.state_bounds[i][0]) for i in range(len(state))]
        new_state = [int(round((self.num_buckets[i] - 1) * ratios[i])) for i in range(len(state))]
        new_state = [min(self.num_buckets[i] - 1, max(0, new_state[i])) for i in range(len(state))]
        return new_state
