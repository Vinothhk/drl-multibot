...
class DQNEnvironmentThree(Node):
    def __init__(self):
        super().__init__('dqn_environment_3')
        ...
        ************************************************************"""
        ...
        self.cmd_vel_pub = self.create_publisher(Twist, '/bot_3/cmd_vel', qos)

        # Initialise subscribers
        self.goal_pose_sub = self.create_subscription(Pose,'bot_3/goal_pose',self.goal_pose_callback,qos)
        self.odom_sub = self.create_subscription(Odometry,'bot_3/odom',self.odom_callback,qos)
        self.scan_sub = self.create_subscription(LaserScan,'bot_3/scan',self.scan_callback,qos_profile=qos_profile_sensor_data)

        # Initialise client
        self.task_succeed_client = self.create_client(Empty, 'bot_3/task_succeed')
        self.task_fail_client = self.create_client(Empty, 'bot_3/task_fail')

        # Initialise servers
        self.dqn_com_server = self.create_service(Dqn, 'bot_3/dqn_com', self.dqn_com_callback)
...
def main(args=None):
    ...
    dqn_environment_3 = DQNEnvironmentThree()
    rclpy.spin(dqn_environment_3)
    dqn_environment_3.destroy()
    rclpy.shutdown()
