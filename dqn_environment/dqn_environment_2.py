...
class DQNEnvironmentTwo(Node):
    def __init__(self):
        super().__init__('dqn_environment_2')
        ...
        ************************************************************"""
        ...
        self.cmd_vel_pub = self.create_publisher(Twist, '/bot_2/cmd_vel', qos)

        # Initialise subscribers
        self.goal_pose_sub = self.create_subscription(Pose,'bot_2/goal_pose',self.goal_pose_callback,qos)
        self.odom_sub = self.create_subscription(Odometry,'bot_2/odom',self.odom_callback,qos)
        self.scan_sub = self.create_subscription(LaserScan,'bot_2/scan',self.scan_callback,qos_profile=qos_profile_sensor_data)

        # Initialise client
        self.task_succeed_client = self.create_client(Empty, 'bot_2/task_succeed')
        self.task_fail_client = self.create_client(Empty, 'bot_2/task_fail')

        # Initialise servers
        self.dqn_com_server = self.create_service(Dqn, 'bot_2/dqn_com', self.dqn_com_callback)
    ...
    *******************************************************************************"""
    ...
    def odom_callback(self, msg):
        ...
        _, _, self.last_pose_theta = self.eulerfromquaternion(msg.pose.pose.orientation)
    ...
    def get_state(self):
        ...
        if self.goal_distance < 0.20:  # unit: m
            ...
            self.local_step = 0
            req = Empty.Request()
            while not self.task_succeed_client.wait_for_service(timeout_sec=3.0):
        ...
        if self.min_obstacle_distance < 0.13:  # unit: m
            ...
            self.local_step = 0
            req = Empty.Request()
            while not self.task_fail_client.wait_for_service(timeout_sec=3.0):
                self.get_logger().info('service not available, waiting again...')
...
def main(args=None):
    ...
    dqn_environment_2 = DQNEnvironmentTwo()
    rclpy.spin(dqn_environment_2)
    dqn_environment_2.destroy()
    rclpy.shutdown()
